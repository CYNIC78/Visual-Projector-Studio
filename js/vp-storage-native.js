...
(function () {
    ...
    // RAM-first desktop state. Regular JSON/text documents live here until Save.
    // Asset binaries, Asset Studio output and world/game package operations still write immediately.
    const _ramDocuments = new Map();
    const _dirtyDocuments = new Map();
    let _isFlushing = false;
    let _allowNativeExit = false;
    let _unsavedDialogPromise = null;
    ...
    const paths = {
        ...
        assetDepth: () => join(WORLD_ROOT(), 'assets', 'depth'),
        ...
        assetStudio: () => join(WORLD_ROOT(), 'asset-studio.json'),
    ...
    function sanitizeAssetForStorage(asset) {
        ...
        return {
            ...
            depthMap: asset.depthMap ? clonePlain(asset.depthMap) : null,
            focusViewport: asset.focusViewport ? clonePlain(asset.focusViewport) : null,
            mime,
    ...
    async function ramReadJson(path, fallback = null) {
        await ensureDirs();
        const key = norm(path);
        if (_ramDocuments.has(key)) return clonePlain(_ramDocuments.get(key).value);
        const value = await readJson(path, fallback, false);
        _ramDocuments.set(key, { type: 'json', value: clonePlain(value) });
        return clonePlain(value);
    }

    async function ramWriteJson(path, data) {
        await ensureDirs();
        const key = norm(path);
        const value = clonePlain(data);
        const doc = { type: 'json', value };
        _ramDocuments.set(key, doc);
        _dirtyDocuments.set(key, doc);
        notifyDirtyChanged();
        return clonePlain(value);
    }

    async function ramReadText(path, fallback = '') {
        await ensureDirs();
        const key = norm(path);
        if (_ramDocuments.has(key)) return String(_ramDocuments.get(key).value || '');
        const value = await readText(path, fallback, false);
        _ramDocuments.set(key, { type: 'text', value: String(value || '') });
        return String(value || '');
    }

    async function ramWriteText(path, text) {
        await ensureDirs();
        const key = norm(path);
        const doc = { type: 'text', value: String(text || '') };
        _ramDocuments.set(key, doc);
        _dirtyDocuments.set(key, doc);
        notifyDirtyChanged();
        return doc.value;
    ...
    function isDirty() { return _dirtyDocuments.size > 0; }

    function notifyDirtyChanged() {
        const detail = { dirty: isDirty(), count: _dirtyDocuments.size, worldId: _activeWorldId };
        try { window.dispatchEvent(new CustomEvent('vp-storage-dirty-changed', { detail })); } catch {}
        try { window.VP_HUB?.emit?.('project:dirty-changed', detail, { moduleId: 'world' }); } catch {}
    }
    ...
    async function saveChanges() {
        ...
        _isFlushing = true;
        try {
            // Snapshot first: edits made during a save remain dirty for the next save.
            const batch = Array.from(_dirtyDocuments.entries());
            for (const [path, doc] of batch) {
                await ensureDir(dirname(path));
                if (doc.type === 'json') await FS.writeFile(path, JSON.stringify(doc.value, null, 2));
                else await FS.writeFile(path, String(doc.value || ''));
                if (_dirtyDocuments.get(path) === doc) _dirtyDocuments.delete(path);
            }
            notifyDirtyChanged();
            try {
                window.VP_HUB?.emit?.('project:saved', {
                    worldId: _activeWorldId,
                    savedCount: batch.length,
                    dirty: isDirty(),
                    dirtyCount: _dirtyDocuments.size,
                    ts: Date.now(),
                }, { moduleId: 'world' });
            } catch {}
            return true;
    ...
    function showUnsavedChangesDialog(reason = 'continue') {
        if (_unsavedDialogPromise) return _unsavedDialogPromise;
        _unsavedDialogPromise = new Promise(resolve => {
            const overlay = document.createElement('div');
            overlay.id = 'vp-unsaved-changes-dialog';
            overlay.style.cssText = 'position:fixed;inset:0;z-index:2147483647;display:flex;align-items:center;justify-content:center;background:rgba(5,5,9,.72);backdrop-filter:blur(5px);font-family:system-ui,-apple-system,Segoe UI,sans-serif';
            const card = document.createElement('div');
            card.style.cssText = 'width:min(440px,calc(100vw - 32px));border:1px solid #383860;border-radius:14px;background:linear-gradient(180deg,#252540 0,#1e1e2e 54px,#171722 100%);color:#cdd6f4;box-shadow:0 24px 80px rgba(0,0,0,.58),0 0 0 1px rgba(108,95,166,.18);overflow:hidden';
            card.innerHTML = `
                <div style="padding:16px 18px 13px;border-bottom:1px solid #383860;display:flex;align-items:center;gap:11px">
                    <div style="width:30px;height:30px;border-radius:9px;display:grid;place-items:center;background:rgba(108,95,166,.28);border:1px solid rgba(151,137,213,.45);font-size:16px">💾</div>
                    <div><div style="font-size:15px;font-weight:800">Unsaved changes</div><div style="margin-top:2px;color:#a6adc8;font-size:11px">VP Studio</div></div>
                </div>
                <div style="padding:20px 20px 12px">
                    <div style="font-size:14px;line-height:1.5">Save changes before ${reason === 'exit' ? 'closing VP Studio' : 'continuing'}?</div>
                    <div style="margin-top:7px;color:#a6adc8;font-size:12px;line-height:1.45">There are ${_dirtyDocuments.size} changed document${_dirtyDocuments.size === 1 ? '' : 's'} in memory.</div>
                </div>
                <div style="display:flex;justify-content:flex-end;gap:9px;padding:14px 18px 18px">
                    <button data-choice="cancel">Cancel</button>
                    <button data-choice="discard">Don't Save</button>
                    <button data-choice="save" data-primary="1">Save</button>
                </div>`;
            overlay.appendChild(card);
            document.body.appendChild(overlay);
            const buttons = [...card.querySelectorAll('button')];
            buttons.forEach(button => {
                const primary = button.dataset.primary === '1';
                button.style.cssText = `min-width:${primary ? 92 : 96}px;height:34px;padding:0 14px;border-radius:8px;border:1px solid ${primary ? '#9789d5' : '#4b4878'};background:${primary ? 'linear-gradient(180deg,#7c6cb8,#584b91)' : 'linear-gradient(180deg,#313052,#23223d)'};color:#cdd6f4;font:600 12px system-ui;cursor:pointer;box-shadow:inset 0 1px rgba(255,255,255,.10),0 3px 10px rgba(0,0,0,.20)`;
            });
            const finish = choice => {
                document.removeEventListener('keydown', onKey, true);
                overlay.remove();
                _unsavedDialogPromise = null;
                resolve(choice);
            };
            const onKey = event => {
                if (event.key === 'Escape') { event.preventDefault(); finish('cancel'); }
                if (event.key === 'Enter') { event.preventDefault(); finish('save'); }
            };
            buttons.forEach(button => button.addEventListener('click', () => finish(button.dataset.choice)));
            overlay.addEventListener('pointerdown', event => { if (event.target === overlay) finish('cancel'); });
            document.addEventListener('keydown', onKey, true);
            card.querySelector('[data-choice="save"]')?.focus();
        });
        return _unsavedDialogPromise;
    }

    async function confirmUnsavedChanges(reason = 'continue') {
        if (!isDirty()) return true;
        const choice = await showUnsavedChangesDialog(reason);
        if (choice === 'save') { await saveChanges(); return true; }
        if (choice === 'discard') { discardChanges(); return true; }
        return false;
    }
    ...
    async function ensureWorldDirs(worldId = _activeWorldId) {
        ...
        await ensureDir(join(paths.worldRoot(worldId), 'assets', 'depth'));
    ...
    async function bulkPutAssets(assets) {
        ...
        if (!shouldPersist('gallery-assets')) return rows;
    ...
    function makeAssetDepthFileName(tag, ext = '.depth.png') {
        const base = safeName(tag || 'asset') || 'asset';
        return `${base}${ext.startsWith('.') ? ext : '.' + ext}`;
    }

    async function ensureAssetDepthDir() {
        await ensureDirs();
        await ensureDir(paths.assetDepth());
        return paths.assetDepth();
    }

    function getAssetDepthPath(fileOrTag) {
        const value = String(fileOrTag || '').trim();
        const file = value.endsWith('.png') || value.endsWith('.pfm') || value.endsWith('.jpg') || value.endsWith('.jpeg')
            ? value
            : makeAssetDepthFileName(value);
        return join(paths.assetDepth(), safeZipRelPath(file) || makeAssetDepthFileName(file));
    }

    async function deleteAssetDepthSidecar(fileOrTag) {
        if (!fileOrTag) return false;
        await ensureAssetDepthDir();
        const target = getAssetDepthPath(fileOrTag);
        try { await FS.remove(target); return true; }
        catch { return false; }
    }

    async function openAssetDepthFolder() {
        await ensureAssetDepthDir();
        if (window.Neutralino?.os?.open) return Neutralino.os.open(paths.assetDepth());
        return false;
    }
    ...
    async function deleteAsset(tag) {
        ...
        if (rec?.depthMap?.file) await deleteAssetDepthSidecar(rec.depthMap.file);
    }
    ...
    async function bulkDeleteAssets(tags) {
        ...
        const meta = await readJson(paths.assetsMeta(), { items: [] });
        const items = Array.isArray(meta.items) ? meta.items : [];
        const removed = items.filter(x => tagSet.has(String(x.tag)));
        if (!removed.length) return;

        meta.items = items.filter(x => !tagSet.has(String(x.tag)));
        meta.updatedAt = Date.now();
        await writeJson(paths.assetsMeta(), meta);

        for (const rec of removed) {
            if (rec?.file) {
                try { await FS.remove(join(paths.assetFiles(), rec.file)); } catch {}
            }
            if (rec?.depthMap?.file) await deleteAssetDepthSidecar(rec.depthMap.file);
        }
    }
    ...
    async function getCustomCss() {
        const css = await ramReadText(paths.customCss(), '');
        applyCustomCss(css);
        return css;
    }
    ...
    async function setCustomCss(css) {
        applyCustomCss(css);
        return ramWriteText(paths.customCss(), css || '');
    }
    ...
    async function setActiveWorld(worldId) {
        ...
        if (id !== _activeWorldId && !(await confirmUnsavedChanges('world-switch'))) throw new Error('World switch cancelled');
        _ramDocuments.clear();
        _dirtyDocuments.clear();
        notifyDirtyChanged();
        _activeWorldId = id;
    ...
    async function duplicateWorld(worldId, title = null) {
        ...
        for (const item of (meta?.items || [])) {
            ...
            if (item?.depthMap?.file) await copyBinaryIfExists(join(WORLD_ROOT(srcId), 'assets', 'depth', item.depthMap.file), join(WORLD_ROOT(dst.id), 'assets', 'depth', item.depthMap.file));
        }
        return dst;
    }
    ...
    async function getGameState(gameId, fallback = {}) {
        await ensureDirs();
        return ramReadJson(paths.gameState(gameId), fallback);
    }

    async function setGameState(gameId, state) {
        await ensureDirs();
        await ensureDir(paths.gameRoot(gameId));
        return ramWriteJson(paths.gameState(gameId), clonePlain(state || {}));
    }
  ...
  <text x="154" y="108" fill="#a6adc8" font-family="system-ui,Segoe UI,sans-serif" font-size="14">Loaded via api.assets.objectUrl()</text>
    ...
    async function exportAll() {
        return {
            ...
            assetStudio: await Storage.getAssetStudioState(),
    ...
    function getWorldDescriptor(world) {
        if (!world) return null;
        return {
            id: world.id || null,
            title: world.title || world.id || null,
            description: world.description || '',
            active: !!world.active || world.id === _activeWorldId,
            createdAt: world.createdAt || null,
            updatedAt: world.updatedAt || null,
        };
    }

    async function getWorldPublicState() {
        await ensureDirs();
        const backend = getBackendInfo();
        const active = await getActiveWorld().catch(() => null);
        const worlds = await listWorlds().catch(() => []);
        return {
            activeWorldId: _activeWorldId,
            activeWorld: getWorldDescriptor(active),
            ...
            backend: {
                ...
                ready: !!backend.ready,
                dataRoot: backend.dataRoot || null,
                worldRoot: backend.worldRoot || null,
            },
        };
    }

    async function switchWorldForHub(worldId, opts = {}) {
        const previousWorldId = _activeWorldId;
        const id = await setActiveWorld(worldId);
        const state = await getWorldPublicState();
        try {
            window.VP_HUB?.emit?.('world:changed', {
                previousWorldId,
                currentWorldId: id,
        ...
        if (opts.reload === true) setTimeout(() => location.reload(), Math.max(0, Number(opts.reloadDelay) || 250));
        return { ok: true, worldId: id, reloadRequired: true, state };
    }

    function registerWorldHubCommands() {
        ...
        const hasModule = !!info?.modules?.some?.(mod => mod.id === 'world');
        if (!hasModule && hub.registerModule) {
            try { hub.registerModule({ id: 'world', title: 'World / Project Storage', version: '1.0.0' }); }
            catch (err) { console.warn('[VP Native Storage] Hub module registration failed:', err); }
        }

        if (!hasCommand('world:get-state')) hub.handle('world:get-state', () => getWorldPublicState(), { moduleId: 'world' });
        if (!hasCommand('world:list')) hub.handle('world:list', async () => ({ ok: true, worlds: (await listWorlds()).map(getWorldDescriptor) }), { moduleId: 'world' });
        if (!hasCommand('world:get-active')) hub.handle('world:get-active', async () => ({ ok: true, world: getWorldDescriptor(await getActiveWorld()) }), { moduleId: 'world' });
        if (!hasCommand('world:create')) {
            hub.handle('world:create', async (payload = {}) => {
                const world = await createWorld(payload);
                const descriptor = getWorldDescriptor(world);
                try { hub.emit('world:created', descriptor, { moduleId: 'world' }); } catch {}
                if (payload.activate === true) return { ok: true, world: descriptor, switch: await switchWorldForHub(world.id, payload) };
                return { ok: true, world: descriptor };
            }, { moduleId: 'world' });
        }
        if (!hasCommand('world:switch')) {
            hub.handle('world:switch', async (payload = {}) => {
                const id = String(payload.id || payload.worldId || '').trim();
                if (!id) throw new Error('world:switch requires payload.id');
                return switchWorldForHub(id, payload);
            }, { moduleId: 'world' });
        }
        if (!hasCommand('world:rename')) {
            hub.handle('world:rename', async (payload = {}) => {
                const id = String(payload.id || payload.worldId || '').trim();
                const title = String(payload.title || '').trim();
                if (!id || !title) throw new Error('world:rename requires id/title');
                const world = await renameWorld(id, title);
                return { ok: !!world, world: getWorldDescriptor(world) };
            }, { moduleId: 'world' });
        }
        if (!hasCommand('world:open-folder')) {
            hub.handle('world:open-folder', async (payload = {}) => ({ ok: await openWorldFolder(payload.id || payload.worldId || _activeWorldId) !== false }), { moduleId: 'world' });
        }
        if (!hasCommand('world:backup')) {
            hub.handle('world:backup', async (payload = {}) => ({ ok: true, result: await backupWorld(payload.id || payload.worldId || _activeWorldId) }), { moduleId: 'world' });
        }
        if (!hasCommand('world:export')) {
            hub.handle('world:export', async (payload = {}) => ({ ok: true, result: await exportWorld(payload.id || payload.worldId || _activeWorldId) }), { moduleId: 'world' });
        }

        if (!hasCommand('project:get-state')) hub.handle('project:get-state', () => getWorldPublicState(), { moduleId: 'world' });
        if (!hasCommand('project:get-dirty-state')) {
            hub.handle('project:get-dirty-state', () => ({ ok: true, dirty: isDirty(), dirtyCount: _dirtyDocuments.size, worldId: _activeWorldId }), { moduleId: 'world' });
        }
        if (!hasCommand('project:save')) {
            hub.handle('project:save', async () => ({ ok: await saveChanges(), dirty: isDirty(), dirtyCount: _dirtyDocuments.size, worldId: _activeWorldId }), { moduleId: 'world' });
        }
        if (!hasCommand('project:discard-changes')) {
            hub.handle('project:discard-changes', () => ({ ok: discardChanges(), dirty: isDirty(), dirtyCount: _dirtyDocuments.size, worldId: _activeWorldId }), { moduleId: 'world' });
        }
    }
    ...
    const Storage = {
        ...
        shouldPersist,
        isDirty,
        getDirtyCount: () => _dirtyDocuments.size,
        saveChanges,
        discardChanges,
        confirmUnsavedChanges,
        reloadWithUnsavedCheck,

        getGalleryData: () => ramReadJson(paths.galleryData(), null),
        setGalleryData: data => shouldPersist('gallery-meta') ? ramWriteJson(paths.galleryData(), clonePlain(data)) : Promise.resolve(data),

        getConfig: () => ramReadJson(paths.config(), null),
        setConfig: config => shouldPersist('config') ? ramWriteJson(paths.config(), clonePlain(config)) : Promise.resolve(config),
        ...
        getProjectorState: () => ramReadJson(paths.projector(), null),
        setProjectorState: state => shouldPersist('projector-state') ? ramWriteJson(paths.projector(), clonePlain(state)) : Promise.resolve(state),

        getWinGeom: () => ramReadJson(paths.windowGeom(), null),
        setWinGeom: geom => shouldPersist('shell') ? ramWriteJson(paths.windowGeom(), clonePlain(geom)) : Promise.resolve(geom),

        getPanelGeom: () => ramReadJson(paths.panelGeom(), null),
        setPanelGeom: geom => shouldPersist('gallery-meta') ? ramWriteJson(paths.panelGeom(), clonePlain(geom)) : Promise.resolve(geom),

        getShellState: () => ramReadJson(paths.shell(), null),
        setShellState: state => shouldPersist('shell') ? ramWriteJson(paths.shell(), clonePlain(state)) : Promise.resolve(state),

        getAssetStudioState: () => ramReadJson(paths.assetStudio(), null),
        setAssetStudioState: state => shouldPersist('asset-studio') ? ramWriteJson(paths.assetStudio(), clonePlain(state)) : Promise.resolve(state),

        getModelConfig: () => ramReadJson(paths.model(), null),
        setModelConfig: config => shouldPersist('model') ? ramWriteJson(paths.model(), clonePlain(config)) : Promise.resolve(config),

        getSessionState: () => ramReadJson(paths.session(), null),
        setSessionState: state => shouldPersist('session') ? ramWriteJson(paths.session(), clonePlain(state)) : Promise.resolve(state),

        getProfiles: () => ramReadJson(paths.profiles(), null),
        setProfiles: store => shouldPersist('profiles') ? ramWriteJson(paths.profiles(), clonePlain(store)) : Promise.resolve(store),

        getChatStore: () => ramReadJson(paths.chats(), null),
        setChatStore: store => shouldPersist('chats') ? ramWriteJson(paths.chats(), clonePlain(store)) : Promise.resolve(store),

        putAsset,
        bulkPutAssets,
        getAllAssets,
        makeAssetDepthFileName,
        ensureAssetDepthDir,
        getAssetDepthPath,
        deleteAssetDepthSidecar,
        openAssetDepthFolder,
        deleteAsset,
        bulkDeleteAssets,
    ...
    async function reloadWithUnsavedCheck(reason = 'reload') {
        if (!(await confirmUnsavedChanges(reason))) return false;
        _allowNativeExit = true;
        try { window.VP_HUB?.emit?.('app:reload-requested', { reason, worldId: _activeWorldId }, { moduleId: 'world' }); } catch {}
        location.reload();
        return true;
    }
    ...
    document.addEventListener('keydown', async (event) => {
        ...
        if ((event.ctrlKey || event.metaKey) && key === 's') {
            ...
            await saveChanges();
            window.VP?.showToast?.('Изменения сохранены', 'success');
            return;
        }

        // Browser/Chromium's native beforeunload dialog is ugly and cannot be
        // styled. Intercept keyboard reloads and route them through our custom
        // RAM-first unsaved-changes dialog instead. Toolbar/browser-menu reloads
        // may still hit beforeunload, but F5/Ctrl+R are now clean.
        const wantsReload = key === 'f5' || ((event.ctrlKey || event.metaKey) && key === 'r');
        if (wantsReload) {
            event.preventDefault();
            event.stopPropagation();
            await reloadWithUnsavedCheck('reload');
        }
    }, true);

    window.addEventListener('beforeunload', (event) => {
        if (!isDirty() || _allowNativeExit) return;
        event.preventDefault();
        event.returnValue = '';
    });
    ...
    if (window.Neutralino?.events?.on) {
        Neutralino.events.on('windowClose', async () => {
            if (_allowNativeExit) return;
            if (!(await confirmUnsavedChanges('exit'))) return;
            _allowNativeExit = true;
    ...
    Storage.ready()
        .then(() => console.log('[VP Native Storage] enabled — RAM-first desktop saves active (Ctrl+S)'))
