...
(function () {
    ...
    function schedulePersistChatStore(opts = {}) {
        // Draft typing is high-frequency. Keep RAM state instant, but don't serialize/write
        // the whole chat store every ~120ms while the user is typing.
        const reason = typeof opts === 'string' ? opts : (opts?.reason || 'default');
        const delay = reason === 'draft' ? 1000 : 120;
    ...
    function setActiveChatDraft(draft) {
        ...
        touchChatMeta(chat);
        schedulePersistChatStore({ reason: 'draft' });
        emitChatDraftChanged(chat.id, chat.ui.draft, 'chats');
        return true;
    }
    ...
    function setActiveChatMessages(messages) {
        ...
        emitChatMessagesChanged(chat.id, 'set-messages', { count: chat.messages.length });
    ...
    function addActiveChatMessage(message) {
        ...
        schedulePersistChatStore();
        emitChatMessagesChanged(chat.id, 'add-message', { message: getMessageDescriptor(normalized), count: chat.messages.length });
        return normalized;
    }
    ...
    function updateActiveChatMessage(id, patch) {
        ...
        touchChatMeta(chat, { bumpLastMessageAt: true });
        schedulePersistChatStore();
        emitChatMessagesChanged(chat.id, 'update-message', { message: getMessageDescriptor(msg), count: chat.messages.length });
        return msg;
    }
    ...
    function deleteActiveChatMessage(id) {
        ...
        if (chat.messages.length !== before) {
            touchChatMeta(chat, { bumpLastMessageAt: true });
            schedulePersistChatStore();
            emitChatMessagesChanged(chat.id, 'delete-message', { messageId: id, count: chat.messages.length });
            return true;
        }
        return false;
    }
    ...
    async function syncProjectorFromRuntimeViaHub() {
        const chat = getActiveChat();
        if (!chat) return false;
        if (window.VP_HUB?.request) {
            try {
                const res = await window.VP_HUB.request('projector:get-snapshot');
                if (!res?.snapshot) return false;
                chat.projector = normalizeProjectorSnapshot(res.snapshot);
                touchChatMeta(chat);
                schedulePersistChatStore();
                return true;
            } catch (err) {
                console.warn('[VP Chats] Hub projector:get-snapshot failed; using legacy fallback:', err);
            }
        }
        return syncProjectorFromRuntime();
    }

    async function applyActiveChatProjectorToRuntimeViaHub() {
        const chat = getActiveChat();
        if (!chat) return false;
        if (window.VP_HUB?.request) {
            try {
                const res = await window.VP_HUB.request('projector:apply-snapshot', { snapshot: chat.projector, persist: true });
                return !!res?.ok;
            } catch (err) {
                console.warn('[VP Chats] Hub projector:apply-snapshot failed; using legacy fallback:', err);
            }
        }
        return applyActiveChatProjectorToRuntime();
    }

    async function setActiveChatViaHub(chatId, opts = {}) {
        const next = getChatById(chatId);
        if (!next) return false;
        const prev = getActiveChat();
        if (prev && prev.id !== next.id && opts.syncProjector !== false) await syncProjectorFromRuntimeViaHub();
        const previousChatId = prev?.id || null;
        S.chats.activeChatId = next.id;
        if (opts.applyProjector !== false) await applyActiveChatProjectorToRuntimeViaHub();
        schedulePersistChatStore();
        if (previousChatId !== next.id) emitChatActiveChanged(previousChatId, next.id, opts.source || 'hub');
        return true;
    }
    ...
    function setActiveChat(chatId, opts = {}) {
        ...
        schedulePersistChatStore();
        if (previousChatId !== next.id) emitChatActiveChanged(previousChatId, next.id, opts.source || 'chats');
        return true;
    }
    ...
    function hasHubCommand(name) {
        try { return !!window.VP_HUB?.inspect?.().commands?.some?.(cmd => cmd.name === name); }
        catch { return false; }
    }

    function requestUiCommand(name, payload = {}, fallback = null) {
        if (window.VP_HUB?.request && hasHubCommand(name)) {
            window.VP_HUB.request(name, payload, { source: 'chats' })
                .catch((err) => {
                    console.warn(`[VP Chats] Hub ${name} failed; using legacy fallback:`, err);
    ...
    function notifyChatUiChanged() {
        requestUiCommand('projector:update-ui', { reason: 'chats' }, () => VP.updateProjectorUI?.());
        requestUiCommand('gallery:refresh-ui', { reason: 'chats' }, () => VP.gallery?.refreshGalleryPanelUI?.());
        requestUiCommand('shell:close-modals', { reason: 'chats' }, () => VP.shell?.closeShellModals?.());
        requestUiCommand('shell:render', { reason: 'chats' }, () => VP.shell?.render?.());
    }

    function getProfileDescriptor(profile) {
        if (!profile) return null;
        return {
            id: profile.id,
    ...
    function getParticipantDescriptor(participant, chat = null) {
        if (!participant) return null;
        const profile = getProfileById(participant.profileId);
        return {
            id: participant.id,
            profileId: participant.profileId || null,
            name: getParticipantDisplayName(participant, chat) || profile?.name || participant.alias || participant.id,
            alias: participant.alias || null,
            enabled: participant.enabled !== false,
            hasPromptPatch: !!String(participant.promptPatch || '').trim(),
            profile: profile ? getProfileDescriptor(profile) : null,
        };
    }

    function getMessageDescriptor(message) {
        if (!message) return null;
        const text = String(message.clean ?? message.raw ?? message.text ?? '');
        return {
            id: message.id || null,
    ...
    function getChatDescriptor(chat, opts = {}) {
        if (!chat) return null;
        const includeParticipants = opts.includeParticipants !== false;
        const includeScenario = opts.includeScenario !== false;
        return {
            id: chat.id,
            title: chat.title || chat.id,
            kind: chat.kind || 'solo',
            active: S.chats?.activeChatId === chat.id,
            participantCount: Array.isArray(chat.participants) ? chat.participants.length : 0,
            activeSpeakerId: chat.activeSpeakerId || null,
            messageCount: Array.isArray(chat.messages) ? chat.messages.length : 0,
            draftLength: String(chat.ui?.draft || '').length,
            hasDraft: !!String(chat.ui?.draft || '').trim(),
            hasScenario: !!(chat.scenario?.enabled && String(chat.scenario?.text || '').trim()),
            scenario: includeScenario ? {
                enabled: chat.scenario?.enabled !== false,
                title: chat.scenario?.title || '',
                textLength: String(chat.scenario?.text || '').length,
            } : undefined,
            projector: {
                currentTag: chat.projector?.currentTag || null,
                historyCount: Array.isArray(chat.projector?.history) ? chat.projector.history.length : 0,
                playbackMessageCount: Array.isArray(chat.projector?.playbackMessages) ? chat.projector.playbackMessages.length : 0,
            },
            participants: includeParticipants && Array.isArray(chat.participants)
                ? chat.participants.map(p => getParticipantDescriptor(p, chat)).filter(Boolean)
                : undefined,
            createdAt: chat.meta?.createdAt || null,
            updatedAt: chat.meta?.updatedAt || null,
            lastMessageAt: chat.meta?.lastMessageAt || null,
        };
    }

    function getChatsPublicState() {
        ensureStores();
        return {
            activeChatId: S.chats.activeChatId || null,
            chatCount: S.chats.items.length,
            profileCount: S.profiles.items.length,
            activeChat: getChatDescriptor(getActiveChat()),
        };
    }

    function listChatDescriptors(payload = {}) {
        ensureStores();
        ...
        let items = S.chats.items;
        if (query) {
            items = items.filter(chat => `${chat.title || ''} ${chat.note || ''}`.toLowerCase().includes(query));
        }
        items = [...items].sort((a, b) => (b.meta?.lastMessageAt || 0) - (a.meta?.lastMessageAt || 0));
        return {
            total: items.length,
            offset,
            limit,
            items: items.slice(offset, offset + limit).map(chat => getChatDescriptor(chat, { includeParticipants: false, includeScenario: false })),
        };
    }

    function listProfileDescriptors(payload = {}) {
        ensureStores();
        const query = String(payload.query || '').trim().toLowerCase();
        const limitRaw = Number(payload.limit ?? 100);
        const offsetRaw = Number(payload.offset ?? 0);
        const limit = Math.max(1, Math.min(500, Number.isFinite(limitRaw) ? Math.floor(limitRaw) : 100));
        const offset = Math.max(0, Number.isFinite(offsetRaw) ? Math.floor(offsetRaw) : 0);
        let items = S.profiles.items;
        if (query) items = items.filter(profile => `${profile.name || ''} ${profile.description || ''}`.toLowerCase().includes(query));
        items = [...items].sort((a, b) => String(a.name || '').localeCompare(String(b.name || '')));
        return { total: items.length, offset, limit, items: items.slice(offset, offset + limit).map(getProfileDescriptor) };
    }

    function getChatMessagesPublic(chatId = null, payload = {}) {
        const chat = getChatById(chatId || S.chats.activeChatId);
        if (!chat) return { ok: false, error: 'chat-not-found', messages: [] };
        const limitRaw = Number(payload.limit ?? 50);
        const offsetRaw = Number(payload.offset ?? Math.max(0, (chat.messages?.length || 0) - limitRaw));
        const limit = Math.max(1, Math.min(500, Number.isFinite(limitRaw) ? Math.floor(limitRaw) : 50));
        const offset = Math.max(0, Number.isFinite(offsetRaw) ? Math.floor(offsetRaw) : 0);
        const messages = Array.isArray(chat.messages) ? chat.messages : [];
        return {
            ok: true,
            chat: getChatDescriptor(chat, { includeParticipants: false, includeScenario: false }),
            total: messages.length,
            offset,
            limit,
            messages: messages.slice(offset, offset + limit).map(getMessageDescriptor),
        };
    }

    function emitChatActiveChanged(previousChatId, currentChatId, source = 'chats') {
        try {
            window.VP_HUB?.emit?.('chat:active-changed', {
                previousChatId: previousChatId || null,
                currentChatId: currentChatId || null,
                current: getChatDescriptor(getChatById(currentChatId), { includeParticipants: false }),
                source,
            }, { moduleId: 'chats' });
        } catch (err) {
            console.warn('[VP Chats] hub emit chat:active-changed failed:', err);
        }
    }

    function emitChatDraftChanged(chatId, draft, source = 'chats') {
        try {
            window.VP_HUB?.emit?.('chat:draft-changed', {
                chatId: chatId || null,
                draft: String(draft || ''),
            ...
            console.warn('[VP Chats] hub emit chat:draft-changed failed:', err);
        }
    }

    function emitChatMessagesChanged(chatId, reason = 'messages-changed', extra = {}) {
        try {
            const chat = getChatById(chatId);
            window.VP_HUB?.emit?.('chat:messages-changed', {
                chatId: chatId || null,
                reason,
                messageCount: Array.isArray(chat?.messages) ? chat.messages.length : 0,
                ...extra,
            ...
            console.warn('[VP Chats] hub emit chat:messages-changed failed:', err);
        }
    }
    ...
    async function activateChatAsync(chatId, opts = {}) {
        const changed = window.VP_HUB?.request
            ? await setActiveChatViaHub(chatId, { source: opts.source || 'ui', syncProjector: opts.syncProjector !== false, applyProjector: opts.applyProjector !== false })
            : setActiveChat(chatId, opts);
        if (changed) notifyChatUiChanged();
        return changed;
    }

    function activateChat(chatId) {
        // Legacy sync entry kept for old callers. UI/new callers use activateChatAsync.
    ...
    async function openLatestOrCreateChatForProfile(profileId) {
        ...
        if (latest) {
            await activateChatAsync(latest.id);
    ...
    function renderChatsPanel(container) {
        ...
        if (!chats.length) {
            ...
            chats.forEach(chat => {
                ...
                item.addEventListener('click', () => { activateChatAsync(chat.id).catch(err => VP.showToast?.(`Chat switch failed: ${err.message || err}`, 'error')); });
    ...
    function registerChatsHubCommands() {
        ...
        const hasModule = !!info?.modules?.some?.(mod => mod.id === 'chats');
        if (!hasModule && hub.registerModule) {
            try { hub.registerModule({ id: 'chats', title: 'Chats', version: '1.0.0' }); }
            catch (err) { console.warn('[VP Chats] Hub module registration failed:', err); }
        }
        if (!hasCommand('chats:get-state')) hub.handle('chats:get-state', () => getChatsPublicState(), { moduleId: 'chats' });
        if (!hasCommand('chats:list')) hub.handle('chats:list', (payload = {}) => listChatDescriptors(payload), { moduleId: 'chats' });
        if (!hasCommand('chats:get-active')) hub.handle('chats:get-active', () => ({ ok: !!getActiveChat(), chat: getChatDescriptor(getActiveChat()) }), { moduleId: 'chats' });
        if (!hasCommand('chats:get-chat')) {
            hub.handle('chats:get-chat', (payload = {}) => {
                const chat = getChatById(payload.id || payload.chatId);
                return { ok: !!chat, chat: getChatDescriptor(chat) };
            }, { moduleId: 'chats' });
        }
        if (!hasCommand('chats:switch-active')) {
            hub.handle('chats:switch-active', async (payload = {}) => {
                const chatId = String(payload.id || payload.chatId || '').trim();
                if (!chatId) throw new Error('chats:switch-active requires payload.id');
                const ok = await setActiveChatViaHub(chatId, {
                    source: 'hub',
                    syncProjector: payload.syncProjector !== false,
                    applyProjector: payload.applyProjector !== false,
                });
                if (ok) notifyChatUiChanged();
                return { ok, state: getChatsPublicState() };
            }, { moduleId: 'chats' });
        }
        if (!hasCommand('chats:list-profiles')) hub.handle('chats:list-profiles', (payload = {}) => listProfileDescriptors(payload), { moduleId: 'chats' });
        if (!hasCommand('chats:get-profile')) {
            hub.handle('chats:get-profile', (payload = {}) => {
                const profile = getProfileById(payload.id || payload.profileId);
                return { ok: !!profile, profile: getProfileDescriptor(profile) };
            }, { moduleId: 'chats' });
        }
        if (!hasCommand('chats:get-active-messages')) hub.handle('chats:get-active-messages', (payload = {}) => getChatMessagesPublic(null, payload), { moduleId: 'chats' });
        if (!hasCommand('chats:get-messages')) hub.handle('chats:get-messages', (payload = {}) => getChatMessagesPublic(payload.id || payload.chatId, payload), { moduleId: 'chats' });
        if (!hasCommand('chats:get-active-draft')) {
            hub.handle('chats:get-active-draft', () => ({ ok: true, chatId: S.chats.activeChatId || null, draft: getActiveChatDraft() }), { moduleId: 'chats' });
        }
        if (!hasCommand('chats:set-active-draft')) {
            hub.handle('chats:set-active-draft', (payload = {}) => ({ ok: setActiveChatDraft(payload.text ?? payload.draft ?? ''), draft: getActiveChatDraft() }), { moduleId: 'chats' });
        }
        if (!hasCommand('chats:set-active-messages')) {
            hub.handle('chats:set-active-messages', (payload = {}) => {
                const messages = setActiveChatMessages(Array.isArray(payload.messages) ? payload.messages : []);
                return { ok: true, count: messages.length, messages: messages.map(getMessageDescriptor) };
            }, { moduleId: 'chats' });
        }
        if (!hasCommand('chats:add-active-message')) {
            hub.handle('chats:add-active-message', (payload = {}) => {
                const message = addActiveChatMessage(payload.message || payload);
                return { ok: !!message, message: getMessageDescriptor(message) };
            }, { moduleId: 'chats' });
        }
        if (!hasCommand('chats:update-active-message')) {
            hub.handle('chats:update-active-message', (payload = {}) => {
                const id = String(payload.id || payload.messageId || '').trim();
                if (!id) throw new Error('chats:update-active-message requires payload.id');
                const message = updateActiveChatMessage(id, payload.patch || {});
                return { ok: !!message, message: getMessageDescriptor(message) };
            }, { moduleId: 'chats' });
        }
        if (!hasCommand('chats:delete-active-message')) {
            hub.handle('chats:delete-active-message', (payload = {}) => {
                const id = String(payload.id || payload.messageId || '').trim();
                if (!id) throw new Error('chats:delete-active-message requires payload.id');
                return { ok: deleteActiveChatMessage(id), messageId: id };
            }, { moduleId: 'chats' });
        }
        if (!hasCommand('chats:get-active-speaker')) {
            hub.handle('chats:get-active-speaker', () => {
                const chat = getActiveChat();
                const speaker = getActiveSpeaker();
                return { ok: !!speaker, speaker: getParticipantDescriptor(speaker, chat) };
            }, { moduleId: 'chats' });
        }
    }
    ...
    const API = {
        ...
        applyActiveChatProjectorToRuntime,
        syncProjectorFromRuntimeViaHub,
        applyActiveChatProjectorToRuntimeViaHub,
