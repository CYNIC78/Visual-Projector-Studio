...
(function (VP) {
    ...
    function getSubtitlePublicState() {
        return {
            ok: true,
            state: {
                mode: SubtitlePlayer.mode || 'idle',
                activeRole: SubtitlePlayer.activeRole || null,
                queueLength: Array.isArray(SubtitlePlayer.queue) ? SubtitlePlayer.queue.length : 0,
                hasCurrentChunk: !!SubtitlePlayer.currentChunk,
                currentChunkLength: SubtitlePlayer.currentChunk ? String(SubtitlePlayer.currentChunk).length : 0,
                hasHideTimer: !!SubtitlePlayer.hideTimer,
                hasUserCueTimer: !!SubtitlePlayer.userCueTimer,
                streamBufferLength: String(SubtitlePlayer.streamBuffer || '').length,
                pendingAssistantBufferLength: String(SubtitlePlayer.pendingAssistantBuffer || '').length,
                canShow: !!SubtitlePlayer.canShowSubtitles?.(),
            },
        };
    }

    function registerSubtitleHubCommands() {
        ...
        const hasModule = !!info?.modules?.some?.(mod => mod.id === 'subtitles');
        if (!hasModule && hub.registerModule) {
            try { hub.registerModule({ id: 'subtitles', title: 'Subtitle Player', version: '1.0.0' }); }
            catch (err) { console.warn('[VP Subtitles] Hub module registration failed:', err); }
        }

        if (!hasCommand('subtitles:get-state')) {
            hub.handle('subtitles:get-state', () => getSubtitlePublicState(), { moduleId: 'subtitles' });
        }
        if (!hasCommand('subtitles:play')) {
            hub.handle('subtitles:play', (payload = {}) => {
                SubtitlePlayer.play(String(payload.text || ''), payload.role || 'assistant');
                return getSubtitlePublicState();
            }, { moduleId: 'subtitles' });
        }
        if (!hasCommand('subtitles:play-live-user-cue')) {
            hub.handle('subtitles:play-live-user-cue', (payload = {}) => {
                SubtitlePlayer.playLiveUserCue(String(payload.text || ''));
                return getSubtitlePublicState();
            }, { moduleId: 'subtitles' });
        }
        if (!hasCommand('subtitles:push-delta')) {
            hub.handle('subtitles:push-delta', (payload = {}) => {
                // Command exists for tools/games/manual diagnostics. The main chat
                // streaming hot path intentionally stays direct to avoid per-token
                // Hub traffic and subtitle latency.
                SubtitlePlayer.pushDelta(String(payload.delta ?? payload.text ?? ''), payload.role || 'assistant');
                return getSubtitlePublicState();
            }, { moduleId: 'subtitles' });
        }
        if (!hasCommand('subtitles:flush-stream')) {
            hub.handle('subtitles:flush-stream', () => {
                SubtitlePlayer.flushStream();
                return getSubtitlePublicState();
            }, { moduleId: 'subtitles' });
        }
        if (!hasCommand('subtitles:stop')) {
            hub.handle('subtitles:stop', () => {
                SubtitlePlayer.stop();
                return getSubtitlePublicState();
            }, { moduleId: 'subtitles' });
        }
        if (!hasCommand('subtitles:clear')) {
            hub.handle('subtitles:clear', () => {
                SubtitlePlayer.clear();
                return getSubtitlePublicState();
            }, { moduleId: 'subtitles' });
        }
    }
    ...
    VP._subtitle = SubtitlePlayer; // legacy compatibility for older modules
    registerSubtitleHubCommands();
